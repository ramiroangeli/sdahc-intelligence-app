...
const ASSUMPTIONS = [
  ...
  {
    id: 'proposal-funnel-kpi-paused-as-lost',
    label: 'Overview Proposal Funnel KPI — Paused counts as Lost, filtered by created-date FY',
    usedIn: 'Overview → Proposal Funnel (near Deal Activity)',
    pages: ['Overview'],
    category: 'Definitional',
    why: 'A new, deliberately simpler sibling of Sales Funnel\'s Proposal Funnel (see \'funnel-proposal-outcomes\'): every deal that reached Proposal Sent or later (its own track\'s A2 Advisory or B2 Brokerage threshold) AND was created in the selected financial year falls into EXACTLY one of two buckets — Progressed (Won, or still In Progress) or Lost — so Sent always equals Progressed + Lost, within that FY. To make that 2-way split exhaustive, a Paused deal counts as "lost" HERE SPECIFICALLY (RealAggregates.proposalFunnelKpi(), supabase-data.js): in practice a paused deal is treated as dead until/unless it\'s reactivated, so folding it into Lost keeps every proposal-stage-or-beyond deal accounted for instead of vanishing into a third, unlabeled bucket. This is a deliberate exception carved out for this one KPI, NOT a change to the standing \'paused-exclusion\' rule (Paused deals still carry zero value in every revenue/pipeline total elsewhere) or to \'pipeline-funnel-lost-exclusion\' (which keeps Paused excluded from the "sent" cohort of the Sales Funnel\'s own Proposal Funnel, whose sent/progressed/lost intentionally do NOT sum to the same total). Do not "fix" this to match the general exclusion rule without re-reading this note — the mismatch with paused-exclusion is intentional.\n\nFinancial-year filter (added after this KPI first shipped): a ‹ › switcher on the panel (app.js\'s proposalFunnelFyBounds()/wireProposalFunnelFySwitcher()) lets you step between financial years, reusing fiscalYearBounds() (data.js) unchanged with a year-shifted anchor date — no separate date-range logic, and it stays in sync with Settings → Financial Year Start the same as every other FY-aware figure in the app. "Next" is disabled once back at the current FY, since a future FY cannot yet have any created deals. The filter reads each deal\'s CREATED date (created_time in Notion) — there is no distinct "proposal sent on" field in the real schema, so this is a proxy/approximation for when the proposal itself went out, not an exact measure; a deal created in one FY whose proposal was actually sent early in the next FY would still be attributed to the creation-FY here. Both the info tooltip and this entry should keep stating that caveat plainly rather than letting the FY label imply more precision than the underlying field supports.',
  },
  {
